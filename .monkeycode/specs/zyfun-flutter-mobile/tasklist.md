...
- [ ] 11. 实现核心页面
  ...
  - [x] 11.3 实现详情页
    ...
    - 实现收藏和分享功能
  - [x] 11.4 实现播放页
    - [x] 创建 `PlayerPage` 集成播放器组件
    - [x] 实现横屏全屏播放
    - [x] 实现后台播放和 PIP 模式
  - [x] 11.5 实现直播页
    - [x] 创建 `LivePage` 展示直播源和频道列表
    - [x] 实现 EPG 节目单显示
    - [x] 实现频道切换和回看功能 (REQ-003)
  - [x] 11.6 实现解析配置页
    - [x] 创建 `ParsePage` 展示解析接口列表
    - [x] 实现解析接口的增删改查
    - [x] 实现默认解析设置
  - [x] 11.7 实现历史页
    - [x] 创建 `HistoryPage` 展示播放历史
    - [x] 实现历史的续看功能
    - [x] 实现单条删除和批量清空 (REQ-007)
  - [x] 11.8 实现收藏页
    - [x] 创建 `FavoritePage` 展示收藏内容
    - [x] 实现收藏的增删操作
    - [x] 实现分组管理
  - [x] 11.9 实现搜索页
  ...
  - [x] 11.11 实现关于页和 AI 功能页
    - [x] 创建 `AboutPage` 展示应用信息
    - [x] 创建 `AiPage` 实现 AI 推荐功能 (REQ-011)
  - [x]* 11.12 为页面编写集成测试
    - [x] 测试页面路由跳转
    - [x] 测试页面数据加载
    - [x] 测试页面的用户交互流程
